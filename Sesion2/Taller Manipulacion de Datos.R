...
# Si es necesario los nombres de las columnas se especifican de manera análoga a la definicion de colClases con col.names = c()
...
#* Seleccione de los anteriores vuelos todos aquellos que salieron entre la medianoche y las 5 de la mañana
...
# Cuales fueron los vuelos con mayores retrasos?
...
# Calcule una nueva variable que sea la distancia promedio por cada año.
g1 <- group_by(vuelos, year) %>% mutate(Distancia_promedio=mean(distance))

#* Calcule una nueva variable que sea la mediana del tiempo de vuelo por año .

# Summarize-

# Calcule el número de vuelos y el número de destinos únicos para cada origen.
g3 <- group_by(vuelos, origin) %>% summarise(N_Vuelos=n(), N_Destinos=n_distinct(dest)) 

#* Calcule el la distancia mínima promedio y máxima por cada destino
